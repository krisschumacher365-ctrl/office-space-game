...
// ============================================================
//  INITECH: THE GAME — WEB EDITION
//  "What would you say... you DO here?"
//  All Initech references are fictional. Some code is very real.
// ============================================================

// ---- game state (per-session, single player) ---------------
...
const STATE = {
  ...
  scoreLog: [],
  achievements: [],
  // TASK QUEUE (Lumbergh keeps piling them on)
  tasks: [],
  tasksCompleted: 0,
  tasksFailed: 0,
  comboStreak: 0,      // consecutive task completions
  bestCombo: 0,
  miltonAnger: 0,       // 0-10, builds toward fire
  coffeeCups: 0,
  printerAttempts: 0,
  // DAY CLOCK
  clockHour: 8,         // 8 AM start
  clockMinute: 0,
  gameOver: false,
  gameOverReason: null,
  // LAWRENCE
...
function log(msg, type = "normal") {
  STATE.messageLog.push({ msg, type, ts: Date.now() });
  // keep last 200
  if (STATE.messageLog.length > 200) STATE.messageLog.shift();
}
...
const PRINTER_ERRORS = [
  "PC LOAD LETTER",
  "PAPER JAM IN TRAY 2",
  "ERR 0x4F46464943455350414345",
  "TONER LOW (replace never)",
  "FEED ME A STRAY CAT",
  "GURU MEDITATION #00000004.0000AAC0",
  "PRINTER ON FIRE",
  "ERROR: SUCCESS",
  "TASK FAILED SUCCESSFULLY",
  "OUT OF CYAN (you were printing black & white)",
  "WARMING UP... (since 1999)",
  "DRIVER NOT FOUND (it was right there a second ago)",
  "SPOOLING... SPOOLING... SPOOLING...",
  "COVER OPEN (it's not)",
  "REPLACE DRUM UNIT (good luck finding one)",
  "ERR: LUMBERGH_PRIORITY_OVERRIDE",
  "JAM IN SECTION 2B... THE BUILDING SECTION",
  "FATAL: INSUFFICIENT FLAIR",
  "ABORT, RETRY, FAIL?",
...
function lumbergh(action) {
  const q = LUMBERGH[Math.floor(Math.random() * LUMBERGH.length)];
  return q.replace("{{A}}", action);
}
...
function fetchWeather() {
  return new Promise((resolve) => {
    https.get("https://wttr.in/?format=j1", { headers: { "User-Agent": "InitechGame/1.0" } }, (res) => {
      let d = "";
      res.on("data", (c) => { d += c; });
      res.on("end", () => {
        try {
...
function getSystemRecon() {
  STATE.systemRecon = {
    ...
    user: os.userInfo().username, homeDir: os.homedir(),
...
function createTPSReport(withCover) {
  ...
  const content = {
    report: `TPS-${String(num).padStart(4, "0")}`,
    date: new Date().toISOString(),
    author: STATE.player,
    coverSheet: withCover,
    coverSheetType: withCover ? "NEW cover sheet (per memo 2/19)" : "MISSING",
    status: withCover ? "COMPLIANT" : "NON-COMPLIANT",
    body: [
      "1. Verify rounding procedures in payroll module.",
      "2. Environment: Node.js " + process.version + " on " + os.platform(),
      `3. Skim balance: $${STATE.skimBalance.toFixed(6)}`,
    ],
    _meta: { realMachine: os.hostname(), realUser: os.userInfo().username, ts: Date.now() },
  };
  const fp = path.join(dir, `TPS-${String(num).padStart(4, "0")}.json`);
...
function fetchMotivation() {
  return new Promise((resolve) => {
    https.get("https://api.chucknorris.io/jokes/random", { headers: { "User-Agent": "InitechGame/1.0" } }, (res) => {
      let d = "";
      res.on("data", (c) => { d += c; });
      res.on("end", () => {
        try { resolve(JSON.parse(d).value); }
        catch { resolve("The printer is jammed. Motivation unavailable."); }
      });
    }).on("error", () => resolve("Michael Bolton doesn't need motivation. He IS motivation."));
  });
}

function saveGame() {
  const sp = path.join(process.cwd(), "initech_save.json");
  fs.writeFileSync(sp, JSON.stringify({ ...STATE, savedAt: new Date().toISOString() }, null, 2));
  return sp;
}
...
function determineEnding() {
  if (STATE.suspicion >= 80 && STATE.skimBalance > 0) {
    return {
      title: "THE AUDIT",
      text: "The Bobs found the penny-fraction scheme. You panic. You write a letter and slide it under Lumbergh's door. Luckily, Milton burns down the building before anyone reads it. Insurance covers everything. The money disappears.\n\n\"I think... we got away with it.\"",
      type: "bad",
    };
  }
  if (!STATE.buildingIntact) {
    return {
      title: "THE FIRE",
      text: "Milton, pushed beyond his limits, sets the building on fire. He's later seen on a tropical beach. With a red stapler. And... is that a coconut drink? And YOUR skim money?!\n\n\"I was told I could listen to the radio...\"",
      type: "bad",
    };
  }
  if (STATE.hypnotized && STATE.morale >= 8) {
    return {
      ...
      text: "After hypnotherapy, you stopped caring. You started showing up in a Hawaiian shirt. You gutted fish at your desk. The Bobs promoted you to VP of Product Refactoring.\n\n\"I wouldn't say I've been missing it, Bob.\"",
      type: "good",
    };
  }
  if (STATE.tpsReportsFiled >= 10 && STATE.tpsCoverSheets >= 10) {
    return {
      title: "THE CORPORATE DRONE",
      text: "You filed every TPS report. With cover sheets. Every time. Lumbergh is thrilled. You get a 3% raise and a parking spot. You die inside a little more each day.\n\n\"Yeahhh... that would be greeeaat.\"",
      type: "neutral",
    };
  }
  if (STATE.skimBalance > 1.0) {
    return {
      title: "THE SALAMI SLICER",
      text: `You successfully skimmed $${STATE.skimBalance.toFixed(6)} in penny fractions. It's not a lot. But it's dishonest work. Peter's Off-Shore Coconut Fund thanks you. 🥥\n\n"We're not gonna do anything ILLEGAL..."`,
      type: "good",
    };
  }
  return {
    title: "THE QUITTER",
    text: "You walk out the front door of Initech. The fluorescents fade behind you. You get a construction job. Manual labor. Fresh air. You've never been happier.\n\n\"Frickin' A.\"",
    type: "neutral",
  };
}
...
function postAction() {
  ...
  // Advance the clock — each action is ~30 minutes
  ...
  // Maybe Lumbergh drops a new task (40% chance, max 5 pending)
  let newTask = null;
  const pendingCount = STATE.tasks.filter(t => t.status === "pending").length;
  if (Math.random() < 0.4 && pendingCount < 5) {
  ...
  // Printer chaos — random background errors
  let printerError = null;
  if (STATE.printerAlive && Math.random() < 0.2) {
    printerError = generatePrinterError();
  }

  // Milton anger builds slowly
  ...
  if (STATE.clockHour >= 17 && !STATE.gameOver) {
    ...
    gameOverData = {
      title: "🕐 5:00 PM — YOU MADE IT",
      text: determineEnding().text,
      type: "survived",
...
app.get("/api/init", async (req, res) => {
  ...
  msgs.push({ type: "system", text: `📡 System recon: ${STATE.systemRecon.hostname} | ${STATE.systemRecon.cpus} CPUs | ${STATE.systemRecon.totalMemGB} GB RAM` });
  msgs.push({ type: "lumbergh", text: lumbergh("come in on Saturday") });

  // Give the player their first task right away
  const firstTask = generateTask();
  msgs.push({ type: "warning", text: `📬 First assignment: "${firstTask.text}" (Go to: ${firstTask.scene})` });

  res.json({ state: STATE, msgs, achievementDefs: ACHIEVEMENT_DEFS, pendingTasks: STATE.tasks.filter(t => t.status === "pending") });
});
...
// ---- CUBICLE -----------------------------------------------
app.post("/api/cubicle", (req, res) => {
  visit("cubicle");
  const { choice } = req.body;
  const msgs = [];

  if (choice === 1) {
    const fp = createTPSReport(true);
    addScore(30, "📋 TPS Report filed (with cover)");
    const tc = completeTask("cubicle");
    msgs.push({ type: "good", text: "✅ TPS Report filed WITH cover sheet. (+30 pts)" });
    msgs.push({ type: "system", text: `Real file created: ${fp}` });
    ...
    const fp = createTPSReport(false);
    addScore(40, "🚫 TPS Report filed (rebel — no cover)");
    const tc = completeTask("cubicle");
    msgs.push({ type: "bad", text: "⚠️ TPS Report filed WITHOUT cover sheet! (+40 pts — rebel bonus)" });
    msgs.push({ type: "system", text: `Real file created: ${fp}` });
...
app.post("/api/printer", (req, res) => {
  ...
  if (!STATE.printerAlive) {
    ...
    return res.json({ ...postAction(), msgs });
  }
...
// ---- CHOTCHKIE'S -------------------------------------------
app.post("/api/chotchkies", (req, res) => {
  visit("chotchkies");
  const { choice } = req.body;
  const msgs = [];
  ...
  if (choice === 1) {
    ...
    if (STATE.flairCount >= 37) {
      msgs.push({ type: "good", text: "🏆 You've matched Brian! 37 pieces of flair!" });
    }
    if (tc) msgs.push({ type: "good", text: `📬 Task complete! +${tc.task.pts + tc.bonus} pts` });
  } else if (choice === 2) {
...
app.post("/api/scheme", (req, res) => {
  ...
  if (choice >= 1 && choice <= 3) {
    ...
    const rp = path.join(process.cwd(), "skim_ledger.json");
    fs.writeFileSync(rp, JSON.stringify({
      disclaimer: "FICTIONAL. This is a game.",
      totalSkimmed: STATE.skimBalance,
      destination: "Peter's Off-Shore Coconut Fund 🥥",
      generated: new Date().toISOString(),
    }, null, 2));
    msgs.push({ type: "system", text: `Ledger written to: ${rp}` });
...
// ---- FISHING -----------------------------------------------
app.post("/api/fishing", async (req, res) => {
  visit("fishing");
  const msgs = [];
...
app.post("/api/lawrence", (req, res) => {
  ...
  if (choice === 1) {
    ...
    // Beer + philosophy
    STATE.lawrenceBeers++;
    STATE.coffeeCups = 0; // beer cancels coffee
    ...
    STATE.morale += 1;
    STATE.suspicion += 3; // drinking during work hours
  } else if (choice === 3) {
    // Channel 9
...
app.get("/api/audit", (req, res) => {
  ...
  const auditMsgs = [
    ...
    { type: "bob", text: `Bob Slydell: "${s.cpus} CPUs and ${s.totalMemGB} GB RAM... and they're using it for TPS reports."` },
  ];
  if (tc) auditMsgs.push({ type: "good", text: `📬 Task complete! +${tc.task.pts + tc.bonus} pts` });
  res.json({
    ...postAction(),
    recon: s,
    msgs: auditMsgs,
...
app.get("/api/motivation", async (req, res) => {
  ...
  if (tc) motMsgs.push({ type: "good", text: `📬 Task complete! +${tc.task.pts + tc.bonus} pts` });
  res.json({
    ...postAction(),
    msgs: motMsgs,
  });
...
app.post("/api/bolton", (req, res) => {
  ...
  if (choice === 1) {
    ...
    msgs.push({ type: "system", text: `Michael: "This is real code. On ${os.hostname()}. Running Node ${process.version}."` });
    STATE.suspicion += 5;
  } else {
...
app.post("/api/save", (req, res) => {
  const sp = saveGame();
  res.json({ ...postAction(), msgs: [{ type: "good", text: `💾 Game saved to: ${sp}` }] });
});
...
app.post("/api/end", (req, res) => {
  ...
  saveGame();
  res.json({ state: STATE, ending, achievementDefs: ACHIEVEMENT_DEFS });
});

// ---- start server ------------------------------------------
const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`\n  ☕ INITECH INTRANET running at http://localhost:${PORT}`);
  console.log("  \"Yeahhh... if you could go ahead and open that URL, that would be greeeaat.\"\n");
});
